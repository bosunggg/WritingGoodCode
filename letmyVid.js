...
function subtractBackground(i) {
  return abs(currentPix[i + 0] - bgPixs[i + 0]);
...
function draw() {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const diffR = subtractBackground(i+0);
      const diffG = subtractBackground(i+1);
      const diffB = subtractBackground(i+2);
